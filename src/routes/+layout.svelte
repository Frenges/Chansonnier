<script lang="ts">
  // Le layout reçoit data depuis +layout.server.ts
  ...
  // Met à jour pagesStore si data change (SSR -> client hydration)
  ...
  // SW / Dexie / offline : import dynamique côté client
  ...
  onMount(async () => {
    ...
    // import dexie-client dynamically (non-blocking)
    try {
      ...
      // ignore in dev if module missing
    }
  });
  ...
  // navigation helper (use goto to avoid full reload)
...
<nav class="sidebar" aria-label="Navigation">
  <div class="nav-links">
    ...
    <a class="nav-link" href="/index/alphabetique">Index alphabétique</a>
    <a class="nav-link" href="/index/thematique">Index thématique</a>
  </div>
  ...
  {#if $results && $results.length > 0}
    <div class="search-results" role="list">
      {#each $results as r}
        <a class="search-item" href={"/page/" + (r.slug ?? r.id)} on:click|preventDefault={() => openResult(r)}>
          {r.title}
        </a>
      {/each}
    </div>
...
<style>
  /* léger, le gros est dans app.css */
